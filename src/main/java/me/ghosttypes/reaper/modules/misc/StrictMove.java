...
/**
 * StrictMove module - Mitigates lag-back from modules and/or strict anti-cheat.
 * Detects server position corrections and optionally disables modules that might cause them.
 */
public class StrictMove extends ReaperModule {
    ...
    private final Setting<Boolean> alert = sgGeneral.add(new BoolSetting.Builder()
        .name("alert")
        .description("Alerts you when lag-back is detected.")
        .defaultValue(false)
        .build());

    private final Setting<Boolean> toggleCombat = sgGeneral.add(new BoolSetting.Builder()
        .name("toggle-combat")
        .description("Disable combat modules when lag-back is detected.")
        .defaultValue(false)
        .build());

    private final Setting<Boolean> toggleMovement = sgGeneral.add(new BoolSetting.Builder()
        .name("toggle-movement")
        .description("Disable movement modules when lag-back is detected.")
        .defaultValue(false)
        .build());
    ...
    public StrictMove() {
        super(Reaper.CATEGORY, "strict-move", "Mitigate lag-back from modules and/or strict anti-cheat.");
    }
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof PlayerPositionLookS2CPacket) {
            // Ignore position updates right after pearl/chorus teleports
    ...
    private void onPacketSend(PacketEvent.Send event) {
        ...
        if (event.packet instanceof PlayerInteractItemC2SPacket packet) {
            // Track when player uses pearl or chorus fruit to avoid false positives
