...
public class Welcomer extends ReaperModule {
    ...
    public Welcomer() {
        super(Reaper.CATEGORY, "welcomer", "Sends a message when somebody joins or leaves the server.");
    }
        ...
        if ((eventPacket instanceof GameMessageS2CPacket packet)) {
            ...
            if (msg.contains("left")) {
                ...
                for (PlayerEntity player : mc.world.getPlayers()) {
                    if (player.getName().getString().equals(name) || player.getDisplayName().getString().contains(name)) {
                        valid = true;
                        break;
                    }
                }
    ...
    private void queueMsg(String msg) {
        try { Thread.sleep(delay.get() * 1000L); } catch (Exception ignored) {}
