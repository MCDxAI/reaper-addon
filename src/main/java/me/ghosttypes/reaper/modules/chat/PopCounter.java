...
public class PopCounter extends ReaperModule {
    ...
    // General
    private final Setting<Boolean> popAlerts = sgGeneral.add(new BoolSetting.Builder()
        .name("pop-alerts")
        .description("Enable global pop notifications")
        .defaultValue(false)
        .build());

    private final Setting<Boolean> deathAlerts = sgGeneral.add(new BoolSetting.Builder()
        .name("death-alerts")
        .description("Enable global death alerts.")
        .defaultValue(false)
        .build());

    private final Setting<Boolean> removeAlerts = sgGeneral.add(new BoolSetting.Builder()
        .name("remove-alerts")
        .description("Enable global entity removal alerts.")
        .defaultValue(false)
        .build());
    ...
    private final Setting<Boolean> announceOthers = sgGeneral.add(new BoolSetting.Builder()
        .name("announce")
    ...
    public final Setting<Boolean> pmOthers = sgGeneral.add(new BoolSetting.Builder()
        .name("pm")
        .description("Message players when they pop a totem.")
        .defaultValue(false)
        .visible(announceOthers::get)
        .build());

    private final Setting<Integer> announceDelay = sgGeneral.add(new IntSetting.Builder()
        .name("announce-delay")
        .description("How many seconds between announcements.")
        .defaultValue(5)
        .min(1)
        .sliderMax(100)
        .visible(announceOthers::get)
    ...
    private final Setting<Double> announceRange = sgGeneral.add(new DoubleSetting.Builder()
        .name("announce-range")
    ...
    private final Setting<Boolean> dontAnnounceFriends = sgGeneral.add(new BoolSetting.Builder()
        .name("dont-announce-friends")
        .description("Don't announce when your friends pop.")
        .defaultValue(true)
        .build());

    public final Setting<Boolean> doPlaceholders = sgGeneral.add(new BoolSetting.Builder()
        .name("placeholders")
        .description("Enable global placeholders for pop messages.")
        .defaultValue(false)
        .build());

    private final Setting<List<String>> popMessages = sgMessages.add(new StringListSetting.Builder()
        .name("pop-messages")
    ...
    public PopCounter() {
        super(Reaper.CATEGORY, "pop-counter", "Count player's totem pops.");
    }
        ...
        synchronized (totemPops) {
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                if (player.deathTime > 0 || player.getHealth() <= 0) {
                    ...
                    if (deathPops.containsKey(u)) deathPops.removeInt(u);
    ...
    private void onReceivePacket(PacketEvent.Receive event) {
        if (event.packet instanceof EntityStatusS2CPacket packet && packet.getStatus() == 35) {
            ...
            if (e == null) {
                ...
                pl = p;
                isFriend = Friends.get().isFriend(p);
            ...
            if (announceOthers.get() && mc.player.distanceTo(e) <= announceRange.get() && announceWait <= 0) {
    ...
    private String getPopAlert(PlayerEntity p, int pops, boolean died) {
        String popAlert;
        if (died) popAlert = p.getName().getString() + " died after popping " + pops + getPopGrammar(pops);
        else popAlert = p.getName().getString() + " popped " + pops + getPopGrammar(pops);
        return popAlert;
    }
