...
/**
 * Alerts when surround blocks are being broken and can auto-reinforce.
...
public class HoleAlert extends ReaperModule {
    ...
    private final Setting<Boolean> packetPlace = sgGeneral.add(new BoolSetting.Builder()
        .name("packet-place")
        .description("Use packet placement for reinforcing.")
        .defaultValue(false)
        .build()
    );

    private final Setting<Boolean> showBreakerName = sgGeneral.add(new BoolSetting.Builder()
        .name("display-breaker")
        .description("Show the name of who is breaking your surround.")
        .defaultValue(false)
        .build()
    );

    private final Setting<Boolean> reinforce = sgGeneral.add(new BoolSetting.Builder()
        .name("reinforce")
        .description("Automatically place blocks to reinforce broken surround.")
        .defaultValue(false)
        .build()
    );

    private final Setting<Integer> reinforceTries = sgGeneral.add(new IntSetting.Builder()
        .name("tries")
        .description("How many times to try reinforcing.")
        .defaultValue(3)
        .min(1)
        .sliderMax(10)
        .visible(reinforce::get)
        .build()
    );

    private final Setting<Boolean> reinforceBurrow = sgGeneral.add(new BoolSetting.Builder()
        .name("semi-burrow")
        .description("Place an anvil above you when reinforcing.")
        .defaultValue(false)
        .visible(reinforce::get)
        .build()
    );
    ...
    private final Setting<Boolean> rotation = sgGeneral.add(new BoolSetting.Builder()
        .name("rotation")
        .description("Rotate when placing reinforce blocks.")
        .defaultValue(false)
        .visible(reinforce::get)
        .build()
    );
    ...
    private final Setting<Integer> delay = sgGeneral.add(new IntSetting.Builder()
        .name("delay")
        .description("Delay between reinforce attempts in ticks.")
        .defaultValue(1)
        .min(0)
        .sliderMax(10)
        .visible(reinforce::get)
    ...
    private final Setting<Boolean> render = sgRender.add(new BoolSetting.Builder()
        .name("render")
        .description("Render the blocks being reinforced.")
        .defaultValue(true)
        .build()
    );
    ...
    public HoleAlert() {
        super(Reaper.CATEGORY, "hole-alert", "Alerts you when your hole is being broken.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (timer <= 0) {
            ...
            if (!obby.found()) {
                ...
                BlockPos surroundPos = placeAt.getLeft();
                Direction dir = placeAt.getRight();
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof BlockBreakingProgressS2CPacket packet) {
            ...
            if (showBreakerName.get()) {
                var entity = mc.world.getEntityById(packet.getEntityId());
                if (entity != null) {
                    // Updated for 1.21.11: getEntityName() -> getName().getString()
                    msg += " by " + entity.getName().getString();
                }
            }
    ...
    private void onRender(Render3DEvent event) {
        if (render.get() && placeAt != null) {
            BlockPos center = placeAt.getLeft();
            BlockPos down = center.offset(placeAt.getRight());
            BlockPos up = down.up();
