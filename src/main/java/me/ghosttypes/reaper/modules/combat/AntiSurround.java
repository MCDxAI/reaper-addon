...
/**
 * Automatically breaks target's surround blocks.
 *
 * Ported from 1.19.4 to 1.21.11.
 *
 * Features:
 * - Automatic surround breaking with pickaxe
 * - Crystal placement after breaking threshold
 * - Support block placement for crystals
 * - Right-click eating support
 * - Progress rendering
 *
 * Dependencies:
 * - CityUtils (for break position calculation)
 * - Task (for one-time execution)
 */
public class AntiSurround extends ReaperModule {
    ...
    private final Setting<Mode> breakMode = sgGeneral.add(new EnumSetting.Builder<Mode>()
        .name("break-mode")
        .description("The way to break the blocks.")
    ...
    private final Setting<Boolean> rotate = sgGeneral.add(new BoolSetting.Builder()
        .name("rotate")
        .description("Automatically faces towards the blocks being placed.")
        .defaultValue(false)
        .build());

    // Misc settings
    private final Setting<Boolean> rightClickEat = sgMisc.add(new BoolSetting.Builder()
        .name("right-click-eat")
        .description("Stops breaking the block and starts eating EGapple.")
        .defaultValue(false)
        .build());

    private final Setting<Boolean> cancelEat = sgMisc.add(new BoolSetting.Builder()
        .name("cancel-eat")
        .description("Press right button again to stop eating.")
        .defaultValue(true)
        .visible(rightClickEat::get)
        .build());

    private final Setting<Boolean> useCrystals = sgMisc.add(new BoolSetting.Builder()
        .name("use-crystals")
        .description("Places crystal forward to the target city block.")
        .defaultValue(true)
        .build());

    private final Setting<Double> breakProgress = sgMisc.add(new DoubleSetting.Builder()
        .name("break-progress")
        .description("Places crystal if break progress of breaking block is higher.")
        .defaultValue(0.979)
        .sliderRange(0, 1)
        .visible(useCrystals::get)
        .build());

    private final Setting<Boolean> support = sgMisc.add(new BoolSetting.Builder()
        .name("support")
        .description("Places obsidian block under potential crystal position.")
        .defaultValue(false)
        .visible(useCrystals::get)
        .build());

    private final Setting<Boolean> ironPickaxe = sgMisc.add(new BoolSetting.Builder()
        .name("iron-pickaxe")
    ...
    private final Setting<SettingColor> sideColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new SettingColor(144, 250, 255, 10))
        .visible(() -> render.get() && (shapeMode.get() == ShapeMode.Sides || shapeMode.get() == ShapeMode.Both))
        .build());

    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder()
        .name("line-color")
        .description("The line color of the target block rendering.")
        .defaultValue(new SettingColor(146, 255, 228))
        .visible(() -> render.get() && (shapeMode.get() == ShapeMode.Lines || shapeMode.get() == ShapeMode.Both))
        .build());

    private final Setting<Boolean> renderProgress = sgRender.add(new BoolSetting.Builder()
        .name("render-progress")
        .description("Renders the breaking progress.")
        .defaultValue(true)
        .build());

    private final Setting<Double> scale = sgRender.add(new DoubleSetting.Builder()
        .name("scale")
        .description("Scale of the breaking progress.")
        .defaultValue(1.5)
        .sliderRange(0.01, 3)
    ...
    public void onTick(TickEvent.Post event) {
        ...
        // Find break position (only once)
        if (breakPos == null) breakPos = CityUtils.getBreakPos(target);
        if (breakPos == null || mc.world.getBlockState(breakPos).isAir()) {
            if (breakPos == null) error("No break pos found.");
            toggle();
            return;
        }

        // Execute break mode logic
                        ...
                        supportTask.run(() -> {
                            ...
                            mc.interactionManager.interactBlock(
                                ...
                                new BlockHitResult(
                                    ...
                                    Direction.DOWN,
                                    CityUtils.getCrystalPos(breakPos, support.get()),
                                    true
                                )
                            );
                            InvUtils.swapBack();
                    ...
                    crystalTask.run(() -> {
                        ...
                        mc.interactionManager.interactBlock(
                            ...
                            new BlockHitResult(
                                new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ()),
                                Direction.DOWN,
                                CityUtils.getCrystalPos(breakPos, false),
                                true
                            )
                        );
                        InvUtils.swapBack();
    ...
    public void on2DRender(Render2DEvent event) {
        ...
        if (NametagUtils.to2D(pos, scale.get())) {
            ...
            progress = String.format("%.2f", ((ClientPlayerInteractionManagerAccessor) mc.interactionManager).meteor$getBreakingProgress()) + "%";
            if (progress.equals("0.00%")) progress = "";
            TextRenderer.get().render(
                progress,
                -TextRenderer.get().getWidth(progress) / 2.0,
                0.0,
                (crystalTask.isCalled() ? new Color(106, 255, 78, 255) : new Color(255, 255, 255, 255))
            );
