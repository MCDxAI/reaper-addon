...
/**
 * Smart hole filling targeting enemies.
 * Fills holes near enemies to prevent them from entering safe positions.
 *
...
public class SmartHoleFill extends ReaperModule {
    ...
    private final Setting<Integer> delay = sgGeneral.add(new IntSetting.Builder()
        .name("fill-delay")
        .description("Delay between filling holes.")
        .defaultValue(1)
        .min(0)
        .sliderMax(10)
        .build());
    ...
    private final Setting<Integer> targetRange = sgGeneral.add(new IntSetting.Builder()
        .name("target-range")
        .description("How far to target players from.")
        .defaultValue(4)
        .min(1)
        .sliderMax(10)
        .build());
    ...
    // Ranges
    private final Setting<Integer> selfHoleRangeH = sgRange.add(new IntSetting.Builder()
        .name("self-range-horizontal")
        .description("Horizontal place range.")
        .defaultValue(4)
        .min(0)
        .sliderMax(10)
        .build());

    private final Setting<Integer> selfHoleRangeV = sgRange.add(new IntSetting.Builder()
        .name("self-range-vertical")
        .description("Vertical place range.")
        .defaultValue(2)
        .min(0)
        .sliderMax(10)
        .build());

    private final Setting<Integer> targetHoleRange = sgRange.add(new IntSetting.Builder()
        .name("target-hole-range")
        .description("How close the target needs to be to the hole.")
        .defaultValue(2)
        .min(0)
        .sliderMax(10)
        .build());
    ...
    // Pause
    public final Setting<Boolean> pauseOnEat = sgPause.add(new BoolSetting.Builder()
        .name("pause-on-eat")
        .description("Pauses while eating.")
        .defaultValue(true)
        .build());

    public final Setting<Boolean> pauseOnDrink = sgPause.add(new BoolSetting.Builder()
        .name("pause-on-drink")
    ...
    public final Setting<Boolean> pauseOnGap = sgPause.add(new BoolSetting.Builder()
        .name("pause-on-gap")
        .description("Pauses while holding a gap.")
        .defaultValue(false)
        .build());
    ...
    public SmartHoleFill() {
        super(Reaper.CATEGORY, "smart-holefill", "Hole fill but smart - fills holes near enemies to prevent safe positioning");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (delayTimer <= 0) {
            ...
            // Get all nearby holes
            List<BlockPos> holes = BlockHelper.getHoles(mc.player.getBlockPos(), selfHoleRangeH.get(), selfHoleRangeV.get());
            if (debug.get()) info("Starting list size: " + holes.size());

            // Remove holes that are too close to the target (we want to fill holes they could use)
            holes.removeIf(hole -> BlockHelper.distanceBetween(target.getBlockPos(), hole) <= targetHoleRange.get());
            if (debug.get()) info("List size after range check: " + holes.size());

            renderBlocks.addAll(holes);
            int filled = 0;
            for (BlockPos hole : holes) {
                // Place block in hole using BlockHelper (which uses Meteor's BlockUtils internally)
                BlockHelper.place(hole, item, rotate.get(), rotatePrio.get());
    ...
    private boolean shouldFill() {
        ...
        switch (fillMode.get()) {
            case Either -> {
                if (Interactions.isInHole() || Interactions.isBurrowed()) shouldFill = true;
            }
