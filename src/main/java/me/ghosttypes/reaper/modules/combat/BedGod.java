...
public class BedGod extends ReaperModule {
    ...
    // general
    public final Setting<Boolean> debug = sgGeneral.add(new BoolSetting.Builder().name("debug").description("Debug logging.").defaultValue(false).build());
    ...
    public BedGod() {
        super(Reaper.CATEGORY, "bed-god", "Automated bed combat for the Nether and End.");
    }

    @Override
    public void onActivate() {
        // Check dimension - beds only explode in Nether and End
        if (mc.world != null && mc.world.getRegistryKey() == World.OVERWORLD) {
            error("Beds don't explode in the Overworld.");
            toggle();
            return;
        }
    ...
    public String getInfoString() {
        if (target != null) {
            String iString = target.getGameProfile().name();
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (autoCraftSync.get() && Interactions.isCrafting()) {
            syncTimer--;
            if (syncTimer <= 0) {
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        timer--;
        if (autoCraft.get()) doAutoCraft();
        doRefill();

        if (autoCraftPause.get() && Interactions.isCrafting()) {
        ...
        // Ignore burrow / ignore self hole checks
        if ((ignoreBurrow.get() && CombatHelper.isBurrowed(target)) ||
    ...
    private int getZeroTick() {
        ...
        if (!CombatHelper.isInHole(target)) {
            if (Interactions.findBedInAll().count() >= zeroTickMinBed.get()) t = Formatter.random(0, 3);
            else t = 5;
        } else {
            if (Interactions.findBedInAll().count() < zeroTickMinBed.get()) {
                t = Formatter.random(2, 4);
            } else {
                t = Formatter.random(0, 3);
                if (CombatHelper.isSelfTrapped(target)) t = 0;
            }
        }
    ...
    private void doAntiString() {
        ...
        for (BlockPos p : BlockHelper.getBlockList(target.getBlockPos(), BlockHelper.BlockListType.Web)) {
            if (BlockHelper.isWeb(p)) {
                if (BlockHelper.isCobweb(p)) {
                    FindItemResult sword = Interactions.findSword();
    ...
    private void doBed(BlockPos pos, Vec3d hitPos, CardinalDirection dir, BlockHitResult hr) {
        ...
        Rotations.rotate(y, Rotations.getPitch(hitPos), () -> {
            ...
            PacketManager.sendInteract(Hand.MAIN_HAND, hr, packetPlace.get());
    ...
    // Auto crafting with 1.21.11 Recipe API
    private void doAutoCraft() {
        if (!autoCraft.get()) return;
        FindItemResult bed = Interactions.findBed();
        if (bed.count() >= autoCraftStopAt.get()) {
        ...
        if (!craftingTable.found() || !wool.found() || !planks.found()) {
            if (debug.get()) warning("cant auto craft, no table wool or planks.");
            if (autoClose.get() && mc.player.currentScreenHandler instanceof CraftingScreenHandler) mc.player.closeHandledScreen();
            return;
        }
        if (wool.count() < 3 || planks.count() < 3) {
            if (debug.get()) warning("cant auto craft, wool or plank count too low");
            if (autoClose.get() && mc.player.currentScreenHandler instanceof CraftingScreenHandler) mc.player.closeHandledScreen();
            return;
        }
        if (Interactions.getEmptySlots() < autoCraftEmptySlots.get() ||
        ...
        if (mc.player.currentScreenHandler instanceof CraftingScreenHandler h) {
            if (debug.get()) info("we're in a crafting table, crafting a bed.");
            doCraft(h);
        } else {
            BlockHelper.BlockPosExtended table = findCraftingTable();
            if (table.nullCheck()) {
    ...
    /**
     * Craft beds using 1.21.11 Recipe API.
     * Pattern from meteor-rejects-v2/AutoCraft.java
     */
    private void doCraft(ScreenHandler h) {
        if (h == null) return;
        CraftingScreenHandler currentScreenHandler = (CraftingScreenHandler) h;

        // 1.21.11 API: Use getOrderedResults() instead of getResultsForGroup()
    ...
    private BlockHelper.BlockPosExtended findCraftingTable() {
        ...
        for (BlockPos p : BlockHelper.getSphere(mc.player.getBlockPos(), 3, 2)) {
            if (!BlockHelper.getBlock(p).equals(Blocks.CRAFTING_TABLE)) continue;
            table.set(p);
        }
        return table;
    }
