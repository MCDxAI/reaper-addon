...
public class SelfTrapPlus extends ReaperModule {
    ...
    private boolean lowArmor() {
        // Check all 4 armor slots (Head, Chest, Legs, Feet) using EquipmentSlot
        if (Interactions.checkThreshold(mc.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.HEAD), smartDura.get())) return true;
        if (Interactions.checkThreshold(mc.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.CHEST), smartDura.get())) return true;
        if (Interactions.checkThreshold(mc.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.LEGS), smartDura.get())) return true;
        if (Interactions.checkThreshold(mc.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.FEET), smartDura.get())) return true;
        return false;
    }
