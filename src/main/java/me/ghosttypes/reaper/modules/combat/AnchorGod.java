...
public class AnchorGod extends ReaperModule {
    ...
    public final Setting<Integer> glowstoneSlot = sgAutoRefill.add(new IntSetting.Builder().name("glowstone-slot").description("Where to move glowstone to.").defaultValue(8).min(1).max(9).sliderMin(1).sliderMax(9).visible(autoRefill::get).build());
    ...
    public AnchorGod() {
        super(Reaper.CATEGORY, "anchor-god", "Automated respawn anchor combat for the overworld.");
    }

    @Override
    public void onActivate() {
        // Check dimension - anchors only explode outside the Nether
        if (mc.world != null && mc.world.getRegistryKey() == World.NETHER) {
            error("Anchors don't explode in the Nether.");
            toggle();
            return;
        }
    ...
    private void onRender(Render3DEvent event) {
        if (render.get()) {
            if (anchorRender != null && anchorRender.getPos() != null) {
                event.renderer.box(anchorRender.getPos(), anchorRender.getSideColor(), anchorRender.getLineColor(), shapeMode.get(), 0);
            }
            if (breakRender != null && breakRender.getPos() != null) {
                event.renderer.box(breakRender.getPos(), breakRender.getSideColor(), breakRender.getLineColor(), shapeMode.get(), 0);
            }
        }
    }
    ...
    private void onTick(TickEvent.Post event) {
        // Check dimension each tick in case of dimension change
        if (mc.world != null && mc.world.getRegistryKey() == World.NETHER) {
            error("Anchors don't explode in the Nether.");
            toggle();
            return;
        ...
        if (target != null) {
            if (target.deathTime > 0 || target.getHealth() <= 0) {
                MeteorClient.EVENT_BUS.post(DeathEvent.KillEvent.get(target, new Vec3d(target.getX(), target.getY(), target.getZ())));
            }
            target = null;
        }
        ...
        // Refilling glowstone
        if (!glowstone.found() && autoRefill.get()) {
        ...
        // Refilling anchors
        if (!anchor.found() && autoRefill.get()) {
            if (strictMotion.get() && CombatHelper.isMoving(mc.player)) return;
    ...
    private void placeAnchor(AnchorPlacement placement) {
        ...
        if (pos == null) {
            if (debug.get()) warning("placeAnchor null pos");
            return;
        }
        ...
        if (!Interactions.isHolding(Items.RESPAWN_ANCHOR)) {
            swapBack = true;
    ...
    private void breakAnchor(AnchorPlacement placement) {
        ...
        if (!BlockHelper.getBlock(pos).equals(Blocks.RESPAWN_ANCHOR)) {
            if (debug.get()) warning("desync detected, resetting breakPos");
            if (BlockHelper.getBlock(pos).equals(Blocks.GLOWSTONE) && antiDesync.get()) {
        ...
        if (renderBreak.get()) {
            breakRender = new Renderers.SimpleAnchorRender(placement.getPos(), renderTimeBreak.get(), sideColorBreak.get(), lineColorBreak.get(), damageColor.get(), fadeFactorBreak.get(), 0.0);
        }
        ...
        if (!strictInv.get() || !strictMotion.get()) {
            Interactions.setSlot(anchor.slot(), false);
        }
    }
    ...
    private AnchorPlacement findBreak() {
        ...
        for (BlockPos p : toScan) {
            if (!BlockHelper.getBlock(p).equals(Blocks.RESPAWN_ANCHOR)) continue;
            if (!placeCheck(p)) continue;
    ...
    private AnchorPlacement getPlacePos() {
        ...
        if (render.get()) {
            anchorRender = new Renderers.SimpleAnchorRender(finalPlacement.getPos(), renderTime.get(), sideColor.get(), lineColor.get(), damageColor.get(), fadeFactor.get(), bestDamage);
        }
        return finalPlacement;
    }
    ...
    private boolean strictCheck(BlockPos anchor) {
        ...
        return !strictRange.get() || !(BlockHelper.distanceTo(anchor) > mc.player.getBlockInteractionRange());
    }
    ...
    private ArrayList<BlockPos> getSphereArray(PlayerEntity target, int xRadius, int yRadius) {
        ...
        if (prediction.get() && CombatHelper.isMoving(target) && !CombatHelper.isInHole(target)) {
            tPos = MathUtil.generatePredict(tPos, target, predictionTicks.get());
        }
        ...
        for (int x = -xRadius; x <= xRadius; x++) {
            for (int y = -yRadius; y <= yRadius; y++) {
                for (int z = -xRadius; z <= xRadius; z++) {
                    ...
                    if (MathHelper.sqrt((float) ((tPos.getX() - p2.getX()) * (tPos.getX() - p2.getX()) + (tPos.getZ() - p2.getZ()) * (tPos.getZ() - p2.getZ()))) <= xRadius
                        && MathHelper.sqrt((float) ((tPos.getY() - p2.getY()) * (tPos.getY() - p2.getY()))) <= yRadius) {
